...
export type TelemetryEventPayload =
  | { type: 'linking'; authorizeUrl: string }
  | { type: 'linked' }
  ...
  interface Window {
    api: {
      ...
      auth: {
        ...
        status: () => Promise<{ id: string; name: string } | null>;
        refresh: () => Promise<{ id: string; name: string }>;
        logout: () => Promise<void>;
      };
      updater: {
        check: () => void;
      ...
      proxy: {
        ...
        start: () => Promise<ProxyStatusAll>;
        configureAndStart: (
          pikaPort: number,
          jartexPort: number,
          bindHost: '0.0.0.0' | '127.0.0.1',
        ) => Promise<ProxyStatusAll>;
        stop: () => Promise<ProxyStatusAll>;
        setPort: (
