...
let proxyStatusPollTimer: ReturnType<typeof setInterval> | undefined;
...
watch(
  ...
  (integratedMode) => {
    setIgnore(integratedMode);
    if (integratedMode) window.api.win.startCursorPoll();
    else window.api.win.stopCursorPoll();
  },
);
...
function handleWindowBlur(): void {
  ...
  updateAnimPlayState();
  clearPendingWake();
  setIgnore(config.integratedMode);
  if (config.integratedMode) window.api.win.startCursorPoll();
}
...
watch(
  () => config.alwaysOnTop,
  (enabled) => {
    window.api.win.setAlwaysOnTop(enabled);
  },
  { immediate: true },
);
...
function setIgnore(ignore: boolean): void {
  ignore = config.integratedMode && ignore;
...
async function syncProxyConnection(): Promise<void> {
  try {
    const status = await window.api.proxy.getStatus();
    if (!status) return;
    const connectedNetwork =
      status.pika.clientCount > 0
        ? 'pikanetwork'
        : status.jartex.clientCount > 0
          ? 'jartexnetwork'
          : null;

    if (players.proxyConnectedNetwork !== connectedNetwork) {
      players.setProxyConnectedNetwork(connectedNetwork);
      if (
        connectedNetwork &&
        config.autoDetectNetwork &&
        connectedNetwork !== config.network
      ) {
        config.network = connectedNetwork as typeof config.network;
        window.api.rpc.setNetwork(connectedNetwork);
      }
      if (connectedNetwork) players.clear();
      else players.clearTeams();
    }
  } catch (error) {
    console.error('Proxy status refresh failed', error);
  }
}
...
onMounted(async () => {
  ...
  void window.api.proxy
    .configureAndStart(config.pikaProxyPort, config.jartexProxyPort, config.proxyBindHost)
    .catch((error: unknown) => {
...
onUnmounted(() => {
  if (proxyStatusPollTimer) {
    clearInterval(proxyStatusPollTimer);
    proxyStatusPollTimer = undefined;
  }
  removeLogListener?.();
  removeShortcutListener?.();
  removeForwardedMoveListener?.();
  window.api.win.stopCursorPoll();
  removeProxyListener?.();
  removeTelemetryListener?.();
  ...
  >
    ...
    <template v-if="postLoadingReady">
      <DiscordLinkModal
        ...
        :authorize-url="discordAuthorizeUrl"
